...
export async function kioskRoutes(app: FastifyInstance) {
    ...
    app.post("/api/kiosk/identify", async (req, reply) => {
        ...
        req.log.info(
            { rawUid, normalizedUid, uidCandidates },
            "badge identify",
        );
        if (!normalizedUid) {
        ...
        if (directUser) {
            if (directUser.is_active !== 1) {
                return reply
                    .code(403)
                    .send({ error: "User disabled", user: directUser });
            }
       ...
       WHERE u.deleted_at IS NULL`,
        ...
        for (const row of rows) {
            ...
            if (
                uidCandidates.some((candidate) => rowCandidates.has(candidate))
            ) {
                userMap.set(row.id, {
                    id: row.id,
                    name: row.name,
    ...
    `,
        ...
        if (pending) {
            return reply.code(409).send({
                error: "Une demande est déjà en attente pour ce badge.",
            });
        }
      ...
      `,
            ...
            return reply
                .code(500)
                .send({ error: "Impossible d'enregistrer la demande." });
        }
    });
    ...
    `,
            ...
            .all(userId, last_end_ts) as Array<{
            product_id: number;
            product_name: string;
            qty: number;
        }>;
    ...
    `,
        ...
        if (!user.email) {
            return reply.code(409).send({
                error: "Aucun email n'est enregistré pour ce compte.",
            });
        }
        ...
        try {
            ...
            if (message.includes("MAIL_NOT_CONFIGURED")) {
                return reply.code(500).send({
                    error: "Envoi email indisponible: configuration SMTP manquante.",
                });
            }
