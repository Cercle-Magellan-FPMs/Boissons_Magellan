...
function decodePngPayload(input: string) {
    ...
    const base64 = value.startsWith(prefix)
...
export async function adminRoutes(app: FastifyInstance) {
    ...
    app.post("/api/admin/products", async (req, reply) => {
        ...
        const { name, is_active, price_cents, initial_qty, image_slug } =
            parsed.data;
        const db = getDB();
      ...
      `,
        ...
        try {
            ...
            if (String(e?.message || "").includes("UNIQUE")) {
                return reply
                    .code(409)
                    .send({ error: "Product name already exists" });
            }
    ...
    app.post("/api/admin/products/:id/image-upload", async (req, reply) => {
        ...
        if (imageBuffer.length > 5 * 1024 * 1024) {
            return reply
                .code(413)
                .send({ error: "PNG trop volumineux (max 5MB)" });
        }
    ...
    `,
        ...
        const previousPath = previousSlug
            ? path.join(imagesDir, `${previousSlug}.png`)
            : null;
        ...
        try {
            ...
            return reply
                .code(500)
                .send({ error: "Impossible d'enregistrer l'image PNG" });
        }
    });
    ...
    `,
        ...
        reply.header(
            "Content-Disposition",
            `attachment; filename=\"stocks-${stamp}.csv\"`,
        );
    ...
    app.post("/api/admin/stocks/import", async (req, reply) => {
        ...
        if (
            ...
            return reply.code(400).send({
                error: "Colonne obligatoire manquante: product_id ou product_name",
            });
        }
    ...
    // --- RESET STOCK (remet tout le stock à zéro avec double confirmation) ---
    app.post("/api/admin/products/reset-stock", async (req, reply) => {
        try {
            requireAdmin(req);
        } catch (e: any) {
            return reply.code(e.statusCode ?? 500).send({ error: e.message });
        ...
        const schema = z.object({
            confirm: z.boolean().refine((v) => v === true, {
                message:
                    "Double confirmation requise : envoyez { confirm: true }",
            }),
        });
        ...
        if (!parsed.success) {
            return reply.code(400).send({
                error:
                    parsed.error.issues[0]?.message ?? "Confirmation manquante",
            });
        }
    ...
    `,
        ...
        if (stocks.length === 0) {
            return reply.send({
                ok: true,
                reset_count: 0,
                message: "Aucun stock à remettre à zéro.",
            });
        }
      ...
      `);
        ...
        return reply.send({
            ok: true,
            reset_count: count,
            message: `${count} produit(s) remis à zéro.`,
        });
    });

    // --- HISTORIQUE DES MOUVEMENTS DE STOCK ---
    app.get("/api/admin/stock-moves", async (req, reply) => {
        try {
            requireAdmin(req);
        } catch (e: any) {
            return reply.code(e.statusCode ?? 500).send({ error: e.message });
