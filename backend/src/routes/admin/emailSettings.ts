...
function writeEnvValues(nextValues: Record<SmtpKey, string>) {
    ...
    for (const key of SMTP_KEYS) {
        if (!seen.has(key))
            lines.push(`${key}=${serializeEnvValue(nextValues[key])}`);
    }
...
function currentSmtpSettings() {
    ...
    return {
        ...
        notify_payment_threshold_cents: Number(
            get("NOTIFY_PAYMENT_THRESHOLD_CENTS") || 500,
        ),
    };
}
...
export async function adminEmailSettingsRoutes(app: FastifyInstance) {
    ...
    app.put("/api/admin/email-settings", async (req, reply) => {
        ...
        const existingPassword =
            existing.get("SMTP_PASS") ?? process.env.SMTP_PASS ?? "";
    ...
    app.post("/api/admin/email-settings/test", async (req, reply) => {
        ...
        const parsed = schema.safeParse(req.body);
        if (!parsed.success)
            return reply.code(400).send({ error: "Invalid test email" });
        ...
        try {
            ...
            if (
                String(error?.message || "").startsWith("MAIL_NOT_CONFIGURED:")
            ) {
                return reply
                    .code(500)
                    .send({ error: "Configuration SMTP incomplete." });
            }
            return reply
                .code(500)
                .send({ error: "Impossible d'envoyer l'email de test." });
        }
    });
}
